function [siai,bot] = REA43_package(data)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
mksz = 800; %redefine marker size for this panel
...
CC = bwconncomp(ff,6); %connected-component labeling
...
CC = bwconncomp(gg,6); %connected-component labeling
numPixels = cellfun(@numel,CC.PixelIdxList);
[~,id] = max(numPixels);
if isempty(id) == 0
...
for k = reg
    ...
    mk(nn) = abs(surv123_ar(k,4) - flow)+0.05;
...
for k = reg2
    ...
    dif2(nn) = abs(surv123_ar(k,4) - flow) * log(d1p(id1)/d1p(id1-1))*...
        log(d2p(id2)/d2p(id2-1))*log(d3p(id3)/d3p(id3-1));
    mk2(nn) = abs(surv123_ar(k,4) - flow)+0.05;
...
if isempty(reg) == 0 || isempty(reg2) == 0
figure('position',[50 50 400 400]);
scatter3(log10(surv123_ar(reg,1)),log10(surv123_ar(reg,2)),log10(surv123_ar(reg,3)),mksz*mk/max(mk),'r.');
hold on
scatter3(log10(surv123_ar(reg2,1)),log10(surv123_ar(reg2,2)),log10(surv123_ar(reg2,3)),mksz*mk2/max(mk2),'b.');
hold off
box on
...
xlabel('Drug-1 Conc. (log10)','fontsize',ft); %x axis label
ylabel('Drug-2 Conc. (log10)','fontsize',ft); %y axis label
zlabel('Drug-3 Conc. (log10)','fontsize',ft); %z axis label
set(gca,'fontsize',ft);
end
